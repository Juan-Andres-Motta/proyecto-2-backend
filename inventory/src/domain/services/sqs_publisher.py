...
class SQSPublisher:
    ...
    def __init__(self, queue_url: str, region: str = "us-east-1"):
        ...
        logger.info(f"Initialized SQSPublisher with queue={queue_url}, region={region}")
    ...
    async def publish_report_generated(
        ...
        logger.debug(f"SQS message content: {json.dumps(message, indent=2)}")
        ...
        async with self.session.client("sqs", region_name=self.region) as sqs:
            try:
                ...
                logger.info(f"Successfully published event for report {report_id} to SQS")

            except Exception as e:
                logger.error(
                    f"Failed to publish report event to SQS for report {report_id}: {e}",
                    exc_info=True
                )
    ...
    async def publish_report_failed(
        ...
        logger.debug(f"SQS failure message: {json.dumps(message, indent=2)}")
        ...
        async with self.session.client("sqs", region_name=self.region) as sqs:
            try:
                ...
                logger.info(
                    f"Successfully published failed event for report {report_id} to SQS"
                )

            except Exception as e:
                logger.error(
                    f"Failed to publish report failed event to SQS for report {report_id}: {e}",
                    exc_info=True,
                )
