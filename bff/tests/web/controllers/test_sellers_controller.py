...
@pytest.mark.asyncio
async def test_create_seller_http_error_from_service():
    """Test seller creation when service returns HTTP error with structured JSON."""
    import httpx
    app = FastAPI()
    app.include_router(router)
    ...
    # Mock HTTPStatusError (e.g., validation error from microservice)
    mock_response = httpx.Response(
        status_code=422,
        json={"error_code": "VALIDATION_ERROR", "message": "Invalid data", "type": "validation_error"}
    )
    http_error = httpx.HTTPStatusError("Error", request=httpx.Request("POST", "http://test"), response=mock_response)

    with patch("web.controllers.sellers_controller.SellerService") as MockSellerService:
        mock_service = MockSellerService.return_value
        mock_service.create_seller = AsyncMock(side_effect=http_error)
...
@pytest.mark.asyncio
async def test_get_sellers_http_error_from_service():
    """Test sellers retrieval when service returns HTTP error (e.g., 503)."""
    import httpx
    app = FastAPI()
    app.include_router(router)

    # Mock HTTPStatusError (e.g., service unavailable)
    mock_response = httpx.Response(
        status_code=503,
    ...
    with patch("web.controllers.sellers_controller.SellerService") as MockSellerService:
        mock_service = MockSellerService.return_value
        mock_service.get_sellers = AsyncMock(side_effect=http_error)
